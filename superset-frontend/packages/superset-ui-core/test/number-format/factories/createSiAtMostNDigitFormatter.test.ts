...
describe('createSiAtMostNDigitFormatter({ n })', () => {
  ...
  it('when n is specified, it formats number in SI format with at most n significant digits', () => {
    ...
    expect(formatter(10100)).toBe('10\u00A0Tsd');
    expect(formatter(111000000)).toBe('110\u00A0Mio');
    ...
    expect(formatter(-10)).toBe('-10');
    expect(formatter(-10001)).toBe('-10\u00A0Tsd');
    expect(formatter(-10101)).toBe('-10\u00A0Tsd');
    expect(formatter(-111000000)).toBe('-110\u00A0Mio');
    expect(formatter(-0.23)).toBe('-230m');
  });
  it('when n is not specified, it defaults to n=3', () => {
    ...
    expect(formatter(1)).toBe('1');
    expect(formatter(10)).toBe('10');
    expect(formatter(10001)).toBe('10.0\u00A0Tsd');
    ...
    expect(formatter(-10)).toBe('-10');
    expect(formatter(-10001)).toBe('-10.0\u00A0Tsd');
    expect(formatter(-10101)).toBe('-10.1\u00A0Tsd');
    expect(formatter(-111000000)).toBe('-111\u00A0Mio');
    expect(formatter(-0.23)).toBe('-230m');
  });
});
